      ...
      {tutorialOpen && snapshot.status === "menu" && (
        <div className="tutorial-scrim">
          <section className="tutorial-panel guide-v2" aria-label="Hướng dẫn chơi">
            ...
            <p className="guide-v2-intro">Bốn thẻ dưới đây là <strong>đúng những vật</strong> đang xuất hiện trong đường chạy. Không cần đoán: màu sắc, chiều cao và cách nhận luôn giống nhau.</p>
            <p className="score-formula-summary"><b>Điểm =</b> quãng đường + vượt vật cản đúng lúc + vòng gió. <strong>Sao không còn tính điểm hoặc tăng combo.</strong></p>
            <div className="guide-action-grid">
              <article className="guide-action-card jump-card"><div className="guide-prop-frame"><img src={GUIDE_JUMP_URL} alt="Đệm dâu hồng thấp" /></div><div className="guide-action-copy"><span>01 · NHẢY</span><h3>Đệm dâu thấp</h3><p>Vật thấp ở mặt đường: chỉ việc nhảy qua để nhận điểm vượt vật cản.</p><div className="key-caps"><kbd>SPACE</kbd><kbd>↑</kbd><b>Nhảy qua</b></div></div></article>
              <article className="guide-action-card slide-card"><div className="guide-prop-frame"><img src={GUIDE_SLIDE_URL} alt="Cổng mây cao có khoảng hở bên dưới" /></div><div className="guide-action-copy"><span>02 · TRƯỢT</span><h3>Cổng mây cao</h3><p>Mây treo cao có khoảng hở: trượt dưới để nhận điểm vượt vật cản.</p><div className="key-caps"><kbd>↓</kbd><b>Trượt dưới</b></div></div></article>
              <article className="guide-action-card shield-card"><div className="guide-prop-frame"><img src={GUIDE_SHIELD_URL} alt="Khiên cầu vồng trong bong bóng xanh" /></div><div className="guide-action-copy"><span>03 · BẢO VỆ</span><h3>Khiên cầu vồng</h3><p>Đổi làn để chạm vào bong bóng. Khiên che được một va chạm trong thời gian ngắn.</p><div className="key-caps"><kbd>←</kbd><kbd>→</kbd><b>Chạm để lấy</b></div></div></article>
              <article className="guide-action-card gust-card"><div className="guide-prop-frame"><img src={GUIDE_GUST_URL} alt="Vòng gió mint" /></div><div className="guide-action-copy"><span>04 · THƯỞNG</span><h3>Vòng gió mint</h3><p>Đổi làn để chạm vào vòng gió: nhận thêm +{SCORE_RULES.gustBasePoints} điểm cố định.</p><div className="key-caps"><kbd>←</kbd><kbd>→</kbd><b>Chạm để lấy</b></div></div></article>
            </div>
            <aside className="score-formula-card" aria-label="Cách tính điểm"><div><span>ĐIỂM TÍNH THẾ NÀO?</span><h3>Bay xa và phản xạ đúng.</h3></div><div className="score-formula-grid"><p><b>Chạy</b><em>+{SCORE_RULES.basePointsPerMeter} điểm/m</em></p><p><b>Nhảy / trượt đúng</b><em>+{SCORE_RULES.hurdleClearPoints} điểm</em></p><p><b>Vòng gió</b><em>+{SCORE_RULES.gustBasePoints} điểm</em></p><p><b>Khiên</b><em>chặn 1 va chạm</em></p></div><small>Sao không còn tính vào điểm, không có combo và không còn mốc thưởng 10 sao.</small></aside>
